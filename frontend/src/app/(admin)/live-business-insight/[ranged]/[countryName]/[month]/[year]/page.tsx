...
//               {/* Header */}
//               <div className="flex flex-col gap-4  xl:items-left xl:justify-between">
//                 {/* Left */}
//                 <h2 className="text-2xl font-bold text-[#414042] whitespace-nowrap">
//                   {/* Performance-based SKU split */}
//                   Live Performance-based SKU Analysis
//                 </h2>
...
const renderAiActionLine = (line: string) => {
  ...
  // If it's not the "Product name - ..." format, render as-is
  if (!m) return <span>{formatBulletLine(line)}</span>;

  const prefix = m[1]; // "Product name - "
  const body = (m[2] || "").trim(); // everything after that

  /**
   * ✅ Split at first "starter" word that begins the insight sentence
   * Covers your cases: The / There / Units / Pricing / Profitability etc.
   */
  const starterRegex =
    /\b(The|There|Units|Pricing|Profitability|Some|Maintain|Review|Check|Reduce|Monitor|A|An)\b/;

  const match = starterRegex.exec(body);
...
const cols: ColumnDef<BIGridRow>[] = [
  { key: 'sNo', header: 'S.No.', width: '70px' },
  { key: 'product', header: 'Product Name', cellClassName: 'text-center', width: '340px' },
  { key: 'salesMix', header: `Sales Mix (${month2Label.split(' ')[0] || 'Current'})`, width: '170px' },

  // ✅ move here (right after Sales Mix)
  ...(isNewRev ? [] : [{ key: 'mixChange', header: 'Sales Mix Change (%)', width: '190px' }]),

  { key: 'unit', header: isNewRev ? `Units (${month2Label || 'Current'})` : 'Unit Growth (%)', width: '170px' },
  { key: 'asp', header: isNewRev ? `ASP (${month2Label || 'Current'})` : 'ASP Growth (%)', width: '170px' },
  { key: 'sales', header: isNewRev ? `Sales (${month2Label || 'Current'})` : 'Net Sales Growth (%)', width: '170px' },
];
