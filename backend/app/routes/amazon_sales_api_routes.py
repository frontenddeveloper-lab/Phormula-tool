...
# import base64, io, os,time, calendar, gzip, csv, logging,  random, urllib.request
# import pandas as pd
...
# import os  , re 
# import pandas as pd
# from app.routes.amazon_api_routes import run_upload_pipeline_from_df, amazon_client, _apply_region_and_marketplace_from_request
# import jwt
...
#     """
#     Parse many 'month' formats and return (year, month_num).

#     Accepted:
#       - 'YYYY-MM' or 'YYYY-M'            -> (2025, 7)
#       - 'YYYY-Month' / 'YYYY-Mon'        -> (2025, 7)
#       - 'Month' / 'Mon' (no year)        -> (current_year, 7)
#       - 'MM' / 'M' (no year)             -> (current_year, 7)
#     """
...
#         pass
#     try:
#         # Accept single-digit month with leading/trailing zeros irregularities
#         if "-" in s:
...
#     # Case 2: YYYY-MonthName / YYYY-Mon
#     if "-" in s:
...
#     # Case 3: MonthName / Mon  -> default year = current
#     name_l = s.lower()
#     for i in range(1, 13):
#         if (calendar.month_name[i].lower() == name_l
#             or calendar.month_abbr[i].lower() == name_l):
...
#     # Case 4: MM / M  -> default year = current
#     if s.isdigit():
#         m = int(s)
#         if 1 <= m <= 12:
#             return y_now, m
...
#     """
#     Accepts:  'YYYY-MM', 'YYYY-M', 'YYYY-July', 'YYYY-Jul',
#               'July', 'Jul', '07', '7'
#     Returns (start_of_month, start_of_next_month) in UTC-naive.
#     """
...
# #     # 1) Fast path: robust ISO8601, including offsets and fractional seconds.
...
# #     # 3) RFC3339 with offset like +00:00 (no fractional seconds)
# #     try:
# #         if len(s) >= 6 and s[-3] == ":" and s[-6] in "+-":
# #             s2 = s[:-3] + s[-2:]  # '...+00:00' -> '...+0000'
...
#     # 1) ISO8601 (handles Z/offsets/fractional)
#     try:
#         s_iso = s.replace("Z", "+00:00") if ("T" in s and ("Z" in s or "+" in s or "-" in s[10:])) else s
#         dt = datetime.fromisoformat(s_iso)
#         if dt.tzinfo is not None:
...
#     # 2) Common explicit formats (assume already UTC)
#     for fmt in (
#         "%Y-%m-%dT%H:%M:%SZ",
#         "%Y-%m-%dT%H:%M:%S.%fZ",
#         "%Y-%m-%d %H:%M:%S",
...
#         try:
#             dt = datetime.strptime(s, fmt)
#             return dt  # treat as UTC-naive
#         except Exception:
#             continue

#     # 3) RFC3339 like 2025-09-01T12:34:56+00:00 (no fractional seconds)
#     try:
#         if len(s) >= 6 and s[-3] == ":" and s[-6] in "+-":
#             s2 = s[:-3] + s[-2:]  # +00:00 -> +0000
#             dt = datetime.strptime(s2, "%Y-%m-%dT%H:%M:%S%z")
...
#     # 4) "Aug 1, 2025 12:01:38 AM PDT" (and variants, incl. no seconds)
#     # split off trailing TZ abbrev if present
#     try:
...
#             left_candidates = [left, left.replace(",", "")]  # with or without comma after day
#             fmts = [
#                 "%b %d %Y %I:%M:%S %p",
#                 "%b %d %Y %I:%M %p",
#             ]
...
#                         # Convert local (TZ-abbrev) to UTC-naive
...
#         "GET_V2_SETTLEMENT_REPORT_DATA_XML",  # add XML
#     )
...
# # ---------- TSV parsing (broadened aliases so city/state/postal can populate) ----------
...
#         # SAFETY: Build row dict without risking IndexError if the line is "short"
#         row = {}
...
#                 "date/time": get(
#                     row,
#                     "posted-date-time", "transaction-date", "posted-date",
#                     "settlement-start-date", "date/time"
#                 ),

#                 # classification
#                 "type": get(row, "transaction-type", "type"),
...
#                 # shipping/buyer/shipment -> city/state/postal (many aliases)
#                 "order city": get(
#                     row,
#                     "ship-city", "shipping-city", "shipment-city", "buyer-city",
#                     "recipient-city", "ship to city", "delivery-city", "order city", "city"
#                 ),
#                 "order state": get(
#                     row,
#                     "ship-state", "shipping-state", "shipment-state", "buyer-state",
#                     "recipient-state", "ship to state", "delivery-state", "order state", "state"
#                 ),
#                 "order postal": get(
#                     row,
#                     "ship-postal-code", "shipping-postal-code", "shipment-postal-code",
#                     "buyer-postal-code", "recipient-postal-code", "ship to postal code",
#                     "delivery-postal-code", "postal-code", "zip", "zipcode", "postcode"
#                 ),
#                 "tax collection model": get(
#                     row, "tax-collection-model", "tax collection model", "tax_collection_model"
#                 ),

#                 # v1 amounts
#                 "product sales": get(row, "product-sales", "total-amount", "total"),
#                 "product sales tax": get(row, "product-sales-tax"),
#                 "postage credits": get(row, "shipping-credits", "postage credits"),
#                 "shipping credits tax": get(row, "shipping-credits-tax"),
#                 "gift wrap credits": get(row, "gift-wrap-credits"),
#                 "giftwrap credits tax": get(row, "gift-wrap-credits-tax"),
#                 "promotional rebates": get(row, "promotional-rebates"),
#                 "promotional rebates tax": get(row, "promotional-rebates-tax"),
#                 "marketplace withheld tax": get(row, "marketplace-withheld-tax"),
#                 "selling fees": get(row, "selling-fees"),
#                 "fba fees": get(row, "fba-fees"),
#                 "other transaction fees": get(row, "other-transaction-fees"),
#                 "other": get(row, "other"),
#                 "total": get(row, "total", "total-amount"),

#                 # explicit advertising column if present
#                 "advertising cost": get(
#                     row, "cost-of-advertising", "cost of advertising", "advertising", "advertisement"
#                 ),
...
#             out.append(mapped)
#         except Exception:
#             # If a bizarre malformed row sneaks in, skip it rather than crashing the whole request.
#             continue
...
#     """
#     Robust parser for GET_V2_SETTLEMENT_REPORT_DATA_XML.
#     We iterate TRANSACTION-level nodes and, for each amount group found under a
#     transaction, emit a single V2-triplet row so the aggregator can bucket it.
#     """
#     try:
...
#     # deep find (first) text anywhere under node for any of the candidate tags
...
#     # find *all* "amount groups" under a transaction node.
#     # a group is any element that contains:
#     #  - an amount number (Amount/TotalAmount)
#     #  - optionally Currency/CurrencyCode
#     #  - optionally AmountType/ChargeType/PriceType
#     #  - optionally AmountDescription/ChargeDescription/PriceDescription
...
#     # Collect transactions (tag endswith 'transaction')
...
#         # very loose date candidates
#         dt    = find_text_deep(
#                     tx,
#                     "posted-date-time","posted-date","posteddatetime","posteddate",
#                     "transaction-date","transactiondate","transaction-posted-date","date"
#                 )
...
#                 # v1 fields left blank (we pivot from V2 triplets below)
#                 "product sales": "",
#                 "product sales tax": "",
#                 "postage credits": "",
...
#     # Fallback: if nothing matched 'transaction', do a coarse sweep for any amounts in the doc
#     if not rows:
#         rows = []
#         for el in root.iter():
#             if norm(el.tag) in ("amount", "totalamount"):
#                 parent = el
#                 # walk up a couple of levels to find context
#                 ctx = el
#                 for _ in range(3):
#                     ctx = ctx.getparent() if hasattr(ctx, "getparent") else None
#                     if ctx is None:
#                         break
#                 ctx = ctx or el  # fallback
...
# # ---------- Aggregate V2 -> SettlementTransaction-like rows ----------
...
#             }
#         # fill missing address/tax model if a later row has it
#         b = buckets[k]
#         for fld, src in (
...
#                 b[fld] = r.get(src)
#         # prefer a currency if bucket missing it
#         if not b.get("currency") and r.get("currency"):
#             b["currency"] = r.get("currency")
#         # keep improving description if a better candidate appears later
...
#         # Case 1: v1 style columns available
#         if any(r.get(k1) for k1 in (
...
#             # explicit v1 advertising column if present
#             add(b, "advertising_cost", _to_decimal(r.get("advertising cost")))
#             if b["total"] is None:
#                 b["total"] = _to_decimal(r.get("total"))
#             continue

#         # Case 2: V2 line -> pivot using amount type/description
...
#         # ---- keyword families ----
#         ship_kw = (
#             "shipping", "delivery", "postage", "carrier",
#             "shippingcharge", "shipping charge", "shippingchargeback", "shipping chargeback",
#         )

#         # Fees that should count as *selling_fees* (non-FBA)
#         selling_fee_kw = (
#             "commission", "referral", "referral fee",
#             "closing fee", "variable closing", "fixed closing",
#             "per item fee", "per-item fee", "service fee",
#             "high volume listing", "listing fee",
#         )

#         # Fees that should be *other_transaction_fees* (NOT selling_fees)
#         other_fee_kw = (
#             "lightning deal", "best deal", "7-day deal", "deal fee",
#             "coupon redemption fee", "coupon redemptions",
#             "refund administration fee", "refund admin fee",
#             "safe-t", "safet", "saf-t",
#             "low inventory level fee", "low-inventory fee",
#         )

#         # FBA/fulfillment-related fees -> fba_fees
#         fba_kw = (
#             "fba", "fulfillment", "fulfilment", "pick & pack",
#             "weight based", "fulfillment per unit", "removal order fee",
#             "prep fee", "labeling fee", "bagging fee"
#         )

#         # These go to 'other' (per your examples)
#         other_hard_kw = (
#             "fba inventory reimbursement", "inventory reimbursement", "reimbursement",
#             "customer return", "general adjustment",
#             "subscription",
#             "to account ending with", "to account ending with:",
#             "disposal fee", "fba disposal fee",
#             "storage fee", "inventory storage", "long term storage", "long-term storage",
#             "aged inventory surcharge", "warehouse storage",
#             "lightning deal", "best deal", "7-day deal", "deal fee",
#             "coupon redemption fee", "coupon redemptions",
#             "refund administration fee", "refund admin fee",
#             "safe-t", "safet", "saf-t",
...
#         # softer "other" catch-alls
#         other_soft_kw = ("other", "goodwill", "misc", "rounding", "adjustment")

#         # special: “ServiceFee + TransactionTotalAmount” lines -> other_transaction_fees
#         is_service_total = (
#             tt in ("servicefee", "service fees", "servicefees", "principal")
#             or ad == "transactiontotalamount"
#             or ad_nospace == "transactiontotalamount"
#         )
...
#             # “ServiceFee / TransactionTotalAmount” → other_transaction_fees
#             add(b, "other_transaction_fees", amt)
#         elif is_other_fee:
#             # storage/deals/coupons/refund-admin/SAFE-T/low-inventory etc.
#             add(b, "other_transaction_fees", amt)
...
#         # 1) platform_fees ONLY for specific "other-transaction" descriptions
#         desc_norm = (b.get("description") or "").strip().lower().replace(" ", "")
#         is_other_txn = (b.get("transaction_type") or "").strip().lower() == "other-transaction"
#         wanted_pf = {"storagefee", "subscriptionfee", "storagerenewalbilling"}
#         if is_other_txn and desc_norm in wanted_pf:
#             b["platform_fees"] = b["total"]
#         else:
#             b["platform_fees"] = None

#         # 2) advertising_cost ONLY for ServiceFee + TransactionTotalAmount
#         tt_norm = (b.get("transaction_type") or "").strip().lower()
#         is_service_fee = tt_norm in {"servicefee", "service fees", "servicefees"}
#         is_tta = (desc_norm == "transactiontotalamount")
#         if is_service_fee and is_tta:
#             b["advertising_cost"] = b["total"]

#         # -----------------------------------

#         # net reimbursement equals computed total for the bucket
#         b["net_reimbursement"] = b["total"]

#         # last-resort description if still empty
#         if not b.get("description"):
#             parts = [p for p in (b.get("transaction_type"), b.get("sku"), b.get("order_id")) if p]
#             b["description"] = " / ".join(parts) if parts else "Amazon settlement line"
...
# def upload():
#     # --- gather inputs from either multipart/form-data or JSON ---
#     # Build df_in from an uploaded file OR from a JSON list of rows
#     df_in = None

#     # 1) Try multipart file first
...
#     else:
#         # 2) Fall back to JSON body with rows/data
...
#     # Helper to read a param from form first, then JSON
...
#     try:
#         user_id = _param("user_id", required=True)  # string or int; pipeline decides
#         ui_country = _param("ui_country", alt="country", required=True, caster=lambda s: str(s).strip())

#         # month can be numeric (1-12 / "01"-"12") or full/abbr month name
...
#                 month_num = int(sm)
#             else:
#                 month_num = _month_to_num(sm)  # uses helper below (now returns int)
...
#         year=int(ui_year),
#         db_url=db_url,      # globals defined at module top
#         db_url_aux=db_url1, # FIX: use db_url1, not env var name
#     )
...
#     """Accepts 'January', 'Jan', case-insensitive; returns 1..12"""
#     import calendar
#     m = mname.strip().lower()
#     for i in range(1, 13):
...
#     """List DONE reports using createdSince/Until (only safe within Amazon's ~90d rule)."""
...
# ) -> list[dict]:
#     """
#     Pages through ALL available DONE reports (newest->older).
#     We do NOT early-stop based on heuristics; pagination or max_pages will end it.
#     """
#     out: list[dict] = []
#     params = {"reportTypes": [rt], "pageSize": page_size, "processingStatuses": ["DONE"]}
#     token = None
...
#     # ✅ Get target marketplace name for filtering
...
#     backfill_pages = int(request.args.get("backfill_pages", "300"))  # increased default

#     # normalize month to YYYY-MM (canonical) or None
#     month_filter = None
#     if month_raw:
#         try:
...
#     # default window (for creating non-settlement types)
#     now = datetime.utcnow()
#     end_dt = now
...
#         # Strategy 1: Recent months (within ~120 days) - use window approach with wider buffer
#         if days_ago <= 120:
#             # Use wider window: start from 30 days before month, end at now or 30 days after month end
#             window_start = month_start - timedelta(days=30)
#             window_end = min(month_end + timedelta(days=30), now)
            
#             # Ensure window_start is not more than 89 days ago (Amazon limit)
...
#         # Strategy 2: Historical months - use deep backfill
#         else:
#             stop_searching_after = month_end + timedelta(days=365)  # search far enough

#             types_to_pull = [
#                 request.args.get("type") or "GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2",
#                 "GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE",     # V1 flat
#                 "GET_V2_SETTLEMENT_REPORT_DATA_XML",           # V2 XML
#             ]
...
#     if not reports_to_fetch:
#         # fallback to most recent DONE or create latest
...
#     # ✅ Make sure this is defined BEFORE the for-loop and outside any if/try blocks
#     processed_reports = 0



#     # Helper: download-decrypt-decompress a single report document
...
#     # Iterate reports (newest first). We'll stop when month coverage is satisfied.
...
#         # Decide parser by sniffing first non-whitespace char
#         snippet = plain.lstrip()[:20]
#         if snippet.startswith(b"<?xml") or snippet.startswith(b"<"):
#             rows = _parse_settlement_xml(plain)
#         else:
#             rows = _parse_settlement_tsv(plain)


#         rep_created = (rep.get("createdTime") or "").rstrip("Z")
#         rep_created_dt = _parse_dt_any(rep_created)

#         # Track coverage and also figure out if we've passed the month
#         min_dt = max_dt = None
#         dts = []
#         if rows:
...
#         # Mark rows with report-created for debugging, then extend
#         for rr in rows:
...
#             # Did this batch include any rows in target month?
#             in_month = any((start_m <= dt < end_m) for dt in dts)
#             if in_month:
#                 have_any_month_rows = True

#             # If we already have month rows, and this report's latest tx date is before the month,
#             # we can safely stop (we've gone past the target month chronologically).
#             if have_any_month_rows and max_dt and max_dt < start_m:
#                 have_passed_month = True

#         # Early stop if coverage is satisfied
...
#     # ✅ DIAGNOSTIC: Log marketplace distribution
#     marketplace_counts = {}
#     for r in aggregated:
...
#     logger.info(f"Filtering for target marketplace: '{target_marketplace_name}'")

#     # ✅ MODIFIED: Filter by marketplace AND month
#     filtered = aggregated
#     month_used = None
    
#     if month_filter:
#         start_m, end_m = _month_bounds(month_filter)
#         month_used = month_filter

#         # 1) Primary filter on aggregated buckets - NOW INCLUDING MARKETPLACE
#         def _in_month_bucket(r):
#             dt = r.get("date_time")
#             marketplace = (r.get("marketplace") or "").strip()
#             date_match = bool(dt and (start_m <= dt < end_m))
#             marketplace_match = (marketplace == target_marketplace_name)
#             return date_match and marketplace_match

#         filtered = [r for r in aggregated if _in_month_bucket(r)]

#         # 2) Fallback: if nothing matched, re-aggregate with marketplace filter
#         if not filtered:
#             def _raw_in_month(rr):
#                 dt = _parse_dt_any(rr.get("date/time"))
#                 marketplace = (rr.get("marketplace") or "").strip()
#                 date_match = bool(dt and (start_m <= dt < end_m))
#                 marketplace_match = (marketplace == target_marketplace_name)
#                 return date_match and marketplace_match
...
#     else:
#         # ✅ NEW: Filter by marketplace only when no month specified
#         def _matches_marketplace(r):
#             marketplace = (r.get("marketplace") or "").strip()
#             return marketplace == target_marketplace_name
        
#         filtered = [r for r in aggregated if _matches_marketplace(r)]
...
#     # ---- monthly totals over all filtered rows (not just preview) ----
#     def D(x):
...
#         "success": True,
#         "marketplace_id": mp,
#         "marketplace_name": target_marketplace_name,  # ✅ Added for clarity
#         "month": month_filter,
...
#         "reports_date_coverage": reports_date_coverage[:10],  # Show first 10
...
# ---------------------------------------------------------------------------
# Amazon Settlements & Finances utilities (cleaned/optimized)
# ---------------------------------------------------------------------------
...
def _aggregate_to_transactions_full(raw_rows: list[dict], user_id: Any) -> tuple[list[dict], set[str]]:
    ...
    for (order_id, sku, dt, tx_type), group_rows in groups.items():
        ...
        for row in group_rows:
            ...
            # Map amount to correct bucket based on type and description
            if amount_type == "itemprice":
                ...
                # Marketplace Facilitator / withheld tax – e.g. MarketplaceFacilitatorTax-Principal
                ...
                if any(hint in desc_lower for hint in ["fba", "fulfillment", "fulfilment", "storage", "disposal"]):
                    totals["fba_fees"] += abs(amount)  # Fees are typically negative
                # Check for selling/referral fees
                ...
                # Fallback - try to categorize by description
        ...
        # Make marketplace_withheld_tax the negative of product_sales_tax (same magnitude, always negative)
        totals["marketplace_withheld_tax"] = -abs(totals["product_sales_tax"])
        
        # ✅ Force promotional_rebates to always be negative
        totals["promotional_rebates"] = -abs(totals["promotional_rebates"])

        # totals["selling_fees"] = -abs(totals["selling_fees"])
        # totals["fba_fees"] = -abs(totals["fba_fees"])
        ...
        agg_record = {
            ...
            "total": total,
            "platform_fees": totals["platform_fees"] if "platform_fees" in totals else Decimal("0"),
            "net_reimbursement": totals["net_reimbursement"] if "net_reimbursement" in totals else Decimal("0"),
        }
...
def _fin_push(
    ...
    Maps tx_type/desc to settlement-style `type` for downstream parity.
    """
    if amount is None:
        return
    q = 0 if quantity is None else quantity

    # Map to settlement type
    ...
    rows.append({
        ...
        "type": settlement_type,              # Settlement-style type
        "marketplace": "",
        "fulfilment": "",
