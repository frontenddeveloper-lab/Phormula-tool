...
#     Body (JSON ya form):
#       - country (required)
#       - marketplace_id (optional; defaults from client/_apply_region...)
#       - transit_time (int, required)
#       - stock_unit (int, required)
#       - source=report (default)  # SP-API se hi fetch hoga
#     """
#     # auth
#     auth_header = request.headers.get('Authorization')
#     if not auth_header or not auth_header.startswith('Bearer '):
...
#     transit_time = int(data.get('transit_time') or request.form.get('transit_time') or 0)
#     stock_unit = int(data.get('stock_unit') or request.form.get('stock_unit') or 0)
...
#     if not country or not mp or not transit_time or not stock_unit:
#         return jsonify({"success": False, "error": "country, marketplace_id, transit_time, stock_unit are required"}), 400

#     try:
#         # 1) Sync from SP-API -> Fee table (reuse your existing flow)
#         rows = []
#         used_source = None
...
#         # 2) Run upload pipeline (Fee -> user_{user}_{country}_table etc.)
...
#             "upload_pipeline": pip,  # {'inserted', 'table', ...}
#             "sample": sample,
#         }), 200
...
#       - country (required)
#       - marketplace_id (optional; defaults from client/_apply_region...)
#       - transit_time (optional int; defaults to 0 if omitted)
#       - stock_unit   (optional int; defaults to 0 if omitted)
#       - source=report (default)  # SP-API se hi fetch hoga
#     """
#     # auth
#     auth_header = request.headers.get('Authorization')
...
#     transit_time = _to_int_or_zero(data.get('transit_time') or request.form.get('transit_time'))
#     stock_unit   = _to_int_or_zero(data.get('stock_unit')   or request.form.get('stock_unit'))

#     mp = data.get("marketplace_id") or request.form.get("marketplace_id") or request.args.get("marketplace_id") or amazon_client.marketplace_id
#     source = (data.get("source") or request.form.get("source") or request.args.get("source") or "report").lower()

#     # ⬇️ only require country + marketplace now
...
#             country=country,
#             marketplace=mp,
#             transit_time=transit_time,   # will be 0 when omitted
#             stock_unit=stock_unit,       # will be 0 when omitted
#             filename_hint="SPAPI_FEE_PREVIEW"
#         )
